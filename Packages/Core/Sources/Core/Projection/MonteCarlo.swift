...
public enum MonteCarlo {
    ...
    public static func run(_ input: MonteCarloInput, calendar: Calendar = .current) -> MonteCarloResult {
        ...
        let annualReturn = NSDecimalNumber(decimal: base.annualReturn.fraction).doubleValue
        let monthlyMean = pow(1 + annualReturn, 1.0 / 12.0) - 1
        let monthlySigma = NSDecimalNumber(decimal: input.annualVolatility.fraction).doubleValue / 12.0.squareRoot()

        let startingBalance = Double(base.startingBalance.minorUnits)
        ...
        for _ in 0..<input.paths {
            ...
            for month in 1...months {
                balance += contribution + (eventsByMonth[month] ?? 0)
                balance *= 1 + monthlyMean + monthlySigma * generator.nextNormal()
                if balance < 0 { balance = 0 }   // 빚으로 굴러가지는 않는다
                if month % 12 == 0 { contribution *= contributionStep }

                if sampleIndex < sampleMonths.count, sampleMonths[sampleIndex] == month {
                    samples[sampleIndex].append(balance)
                    sampleIndex += 1
                }
            }
