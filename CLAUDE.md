...
**새 앱을 시작하거나 같은 실수를 피하려면 [docs/12-lessons-learned.md](docs/12-lessons-learned.md) 를
먼저 읽으세요.** 이 저장소에서 겪은 시행착오를 다음 앱에 가져갈 규칙과 체크리스트로 추린 문서입니다.
...
- **모든 금액은 원화다.** 통화별 입력과 환율 환산은 만들었다가 걷어냈다 — 사용자가
  ...
  이 앱은 `.krw` 하나만 쓴다.
- **기능을 더하기 전에 같은 역할을 하는 것이 이미 있는지 먼저 찾는다** (2026-09-19,
  168번에서 정한 정책). 새 칸·새 화면·새 규칙을 만들기 전에 `grep` 으로 비슷한
  이름·문구·계산을 훑고, 있으면 **그것에 이름을 붙이거나 표시를 더하는 쪽**으로
  간다. "가족 대표" 가 그 예다 — 새 저장 칸 대신 이미 있던 `구성원 순서` 의 맨 위
...
- **출시 일정은 묻지 않고 계속 고친다** (2026-09-22 사용자 요청).
  [로드맵의 "출시 일정"](docs/05-roadmap.md#출시-일정--claude-가-묻지-않고-계속-고친다-2026-09-22-사용자-요청)
  블록이 사실과 어긋나면 **그 자리에서** 고쳐 적고 커밋한다 — 안정화 기준의
  날짜가 바뀌거나, 시계가 다시 돌아가거나, 준비 목록 항목이 끝났을 때다.
  사용자가 말해 주기를 기다리지 않는다.
...
- **앱 이름은 `느린 부자`** 다 (2026-09-24, `느린 부자의 기록` 에서 줄임).
  빌드용 식별자 `SlowRich` 와 번들 ID `com.helpnara.slowrich` 는 **그대로 둔다** —
  번들 ID를 바꾸면 iCloud 컨테이너와 TestFlight 설치본이 남남이 된다.
  화면 상단의 `우리 가족 노후자금 준비` 는 앱 이름이 아니라 **계획 제목**이며
  사용자가 바꿀 수 있는 값이다 (`Plan.title`). 둘을 섞지 않는다.
- **`느린` 시리즈** (2026-09-24 사용자 결정). 앞으로 만드는 앱은 `느린` 으로
  묶는다 — `느린 부자` 가 첫째고, `느린 여백` · `느린 나이테` 가 뒤따른다.
  그래서 이름에 **앱마다 다른 말은 한 단어만** 붙인다. `~의 기록` 처럼 공통이
  될 법한 꼬리는 붙이지 않는다 — 셋 다 `~의 기록` 이 되면 시리즈가 아니라
  같은 이름 셋이 된다. 새 앱을 시작하면 [docs/12-lessons-learned.md](docs/12-lessons-learned.md)
  를 먼저 읽는다.
