...
extension View {
    ...
    func swipeDelete(_ action: @escaping () -> Void) -> some View {
        ...
        swipeActions(edge: .trailing, allowsFullSwipe: false) {
            ...
            // **지우기 역할(`role: .destructive`)을 주지 않는다** (182번 결론).
            // 목록은 그 역할의 미는 버튼을 누르면 자료가 곧 지워질 것으로 보고
            // **그 줄을 먼저 접어 없앤다** — `onDelete` 와 같은 낙관 동작이다.
            // 우리는 확인 창을 띄우느라 자료를 그대로 두므로, 다음 다시 그리기에서
            // 줄이 되돌아왔다. 106 은 그 되돌림이 즉시라 아래 줄만 흔들려 보였고,
            // 107 은 창을 늦춰 되돌림도 늦어지며 정체가 드러났다(사용자 녹화).
            // 역할을 빼면 줄은 그대로 있고 밀기만 닫힌다. 빨강은 색으로 준다.
            Button("삭제", action: action)
                .tint(Color.loss)
        }
    }
