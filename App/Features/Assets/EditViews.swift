        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    Text(isFamilyHead
                         ? "이 사람은 가족 대표입니다. 여기 적은 은퇴 목표가 계획 탭의 은퇴 목표(가족 전체의 궤적)가 됩니다."
                         : "가족 전체의 궤적은 계획 탭의 은퇴 연도(가족 대표의 것)를 쓰고, 이 사람의 궤적과 1페이지 카드는 여기 적은 연도를 씁니다.")
                }
                ...
                Section {
                    ...
                    if member.taxResidency.isSubjectToPFIC {
                        Text("국적이 아니라 세금 신고 의무가 있는 나라입니다. 미국이 들어가면 한국 상장 ETF는 PFIC로 분류되어 세금이 불리합니다 — 해당 종목에 경고를 표시합니다.")
                    } else {
                        Text("국적이 아니라 세금 신고 의무가 있는 나라입니다. 미국이 들어가면 한국 상장 ETF에 PFIC 경고가 붙습니다.")
                    }
                }
                ...
                Section {
                    ...
                    Text("궤적에는 합계가 쓰이고, 저축률 진단에는 본인 부담만 씁니다. 회사가 넣어 주는 돈을 내 저축으로 세면 저축률이 실제보다 높게 나옵니다.")
                }
...
struct AccountEditView: View {
            ...
            Form {
                ...
                if account.kind == .leaseDeposit {
                    Section {
                        ...
                        Text("연 월세가 매매가의 5% 이내면 적정으로 봅니다. 자산 진단의 '월세 적정성' 이 이 두 값으로 판단합니다. 매매가는 비슷한 집의 최근 실거래가를 적으면 됩니다.")
                    }
                }
    ...
    private var returnFooter: String {
        ...
        case .fixed:
            return "전월세보증금·받을 돈은 자라지 않는 돈으로 봅니다. 궤적에서 명목 그대로 남습니다."
        }
    }
}
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    Text("같은 자산군 안에서의 비중입니다. 자산군끼리의 비중은 자산 탭 → 구성원 → 목표 비중에서 정합니다. 정해 두면 매주 점검할 때 어긋난 종목에 배지가 뜹니다.")
                }
                ...
                Section {
                    ...
                    if !canEditValue {
                        ...
                        Text("시세를 가져오지 않습니다. 해외 종목도 원화로 환산한 금액을 적어 주세요. 이 앱의 모든 금액은 원화입니다.")
                    }
                }
