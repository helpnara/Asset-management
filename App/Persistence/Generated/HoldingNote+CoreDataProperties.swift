...
extension HoldingNote {
    ...
    @nonobjc class func fetchRequest() -> NSFetchRequest<HoldingNote> {
        NSFetchRequest<HoldingNote>(entityName: "HoldingNote")
    }

    @NSManaged var id: UUID

    /// 적은 시각. 목록은 **최신이 위**다 — 지금 생각이 먼저 읽혀야 한다.
    @NSManaged var at: Date

    @NSManaged var body: String

    /// 누가 적었나 (`ActorName`, 74번). 가족이 함께 쓰므로 아내분이 적은 줄과
    /// 내가 적은 줄이 섞인다.
    @NSManaged var actor: String

    @NSManaged var holding: Holding?
