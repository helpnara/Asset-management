...
> `느린 부자 - 가계 자산` 처럼 뒤를 붙이세요. 앱 아이콘 아래 표시되는
> 이름은 `CFBundleDisplayName`(= `느린 부자`)이라 여기 이름과 달라도 됩니다.
...
#### 가족을 테스터로 넣을 때 — 외부 그룹이 아니라 내부 그룹에 (2026-09-09)

TestFlight 의 **외부 그룹**(Connect 앱에서 `외부 그룹 · N명의 테스터` 로 보이는
것)에 넣으면 빌드마다 **베타 앱 심사**를 거쳐야 테스터가 설치할 수 있다.
첫 심사는 하루 이틀 걸리고, 그동안 테스터 칸에는 `사용 가능한 빌드 없음` 만
보인다. 실제로 이렇게 하루를 잃었다 — 아내분 폰에서 초대 링크를 눌렀더니
`'cloudkit.zoneshare' 을(를) 열 수 없음 · 필요한 앱이 이 기기에 설치되어 있지
않으며` 가 떴다. 앱이 안 깔려 있으니 당연한 메시지다.

**내부 그룹**은 심사가 없고 빌드가 올라오는 즉시 설치할 수 있다. 대신 팀
구성원이어야 하므로 먼저 사용자로 초대한다:

1. [appstoreconnect.apple.com](https://appstoreconnect.apple.com) → **사용자 및 액세스**
   → **+** → 이름과 Apple ID 메일 → 역할은 **고객 지원** (가장 낮은 권한) → 초대
2. 가족이 메일의 초대를 수락한다 (본인 Apple ID 로 로그인)
3. 앱 → **TestFlight** → **내부 테스팅** 의 그룹 → **테스터 +** → 방금 초대한 사람
   선택. 그룹에 `모든 빌드 자동 배포` 를 켜 두면 새 빌드가 알아서 들어간다
4. 가족 폰에서 TestFlight 앱을 받고, 초대 메일 또는 TestFlight 안에서 설치
5. 그 다음에 메시지의 초대 링크를 **다시** 누른다. 링크는 그대로 살아 있다

초대 링크를 받아들이려면 **폰의 iCloud 계정이 링크를 받은 번호·메일과 같은
사람**이어야 한다 (공유는 초대받은 참가자만 열 수 있게 만든다). TestFlight 에
쓰는 Apple ID 와 iCloud 계정이 달라도 상관없다 — 앱만 깔리면 된다.
...
| 전월세보증금 같은 항목을 `고정`으로 뒀는가 | 매주 물어보는 항목이 줄어든다 |
...
| 언제 | 무엇 | 확인 |
|---|---|---|
| 2026-09-07 | 레코드 타입 15개 (최초) | 앱에서 `마지막 내보내기: 성공` |
| 2026-09-07 | `Plan.updatedAt` 추가분 | `read` 로 Production 에 `CD_updatedAt TIMESTAMP` 확인 |
