...
펼친 책 위의 상승 곡선이고, 색은 블로그 아이콘에서 그대로 가져왔습니다
(`#2F3925` / `#FAF2E5`). 바꾸려면 `Tools/make-icon.py` 를 고쳐서 다시 돌립니다.
...
**블로그 이미지를 그대로 앱 아이콘으로 쓰면 안 되는 이유** — 흔한 함정이라
적어 둡니다.

| 문제 | 왜 |
|---|---|
| 글자가 들어 있다 | 홈 화면 아이콘은 60pt 다. 한글 세 줄은 그 크기에서 읽히지 않는다 (Apple HIG 가 아이콘에 글자를 넣지 말라고 못 박는다) |
| 사진 같은 디테일 | 책등·공책 글씨·액자 속 인물은 60pt 에서 전부 뭉갠다 |
| 안에 원이 그려져 있다 | iOS 가 이미 모서리를 깎는다. 안에 또 원을 그리면 네 귀퉁이가 어중간하게 남아 '스티커'로 보이고, 테두리 원은 잘린다 |
| 실제 책 표지가 보인다 | *The Intelligent Investor* 등은 실존 저작물이다. 앱 아이콘·스크린샷에 알아볼 수 있게 넣으면 상표·저작권 문제가 될 수 있다 |
| 생성형 이미지 | 심사에 바로 걸리지는 않지만, 제출물의 권리는 제출자가 갖고 있어야 한다 |
